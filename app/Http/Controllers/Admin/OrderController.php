...
class OrderController extends Controller
{
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $show_data = Order::latest()->with('shipping', 'status', 'notes');
            if ($request->keyword) {
            ...
            $show_data = Order::where(['order_status' => $order_status->id])->latest()->with('shipping', 'status', 'notes')->paginate(10);
        }
        $users = User::get();
    ...
    // getNote
    public function getNote($orderId)
    {
        $note = OrderNote::where('order_id', $orderId)->first();
        return response()->json(['note' => $note->order_note ?? '']);
    }

    // saveNote
    public function saveNote(Request $request)
    {
        // Find existing note or create new one for this order
        $note = OrderNote::firstOrNew(['order_id' => $request->order_id]);
        $note->order_note = $request->note;
        $note->save();
    ...
    public function product_discount(Request $request)
    {
        ...
        $cart = Cart::instance('pos_shopping')->get($rowId);

        // Keep existing options
        $options = $cart->options->toArray();
        $options['product_discount'] = $discount; // only change this one

        // Update without triggering hash change
    ...
    public function order_update(Request $request)
    {
        ...
        // 1. Get all product IDs currently in cart
        $cartProductIds = Cart::instance('pos_shopping')->content()->pluck('id')->toArray();

        // 2. Delete removed items from OrderDetails
