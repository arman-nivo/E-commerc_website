...
// Fix broken asset URLs that contain an extra `public/` segment
Route::get('public/uploads/{path}', function ($path) {
    return redirect("/uploads/{$path}", 301);
})->where('path', '.*');
...
Route::group(['namespace' => 'Frontend', 'middleware' => ['ipcheck', 'check_refer']], function () {
    ...
    Route::post('site/contact-us', [FrontendController::class, 'contactSubmit'])->name('contact.submit');
...
Route::group(['namespace' => 'Admin', 'middleware' => ['auth', 'lock', 'check_refer'], 'prefix' => 'admin'], function () {
    ...
    // order-note
    Route::get('get-note/{orderId}', [OrderController::class, 'getNote']);
    Route::post('save-note', [OrderController::class, 'saveNote']);
